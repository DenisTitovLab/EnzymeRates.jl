...
- `_dedup_flat!(mechs::Vector)` → `unique!`: mechanisms are canonical at construction (the `Mechanism`/`AllostericMechanism` constructor sorts steps and groups), so dedup is non-mutating structural `==`/`hash` comparison.
