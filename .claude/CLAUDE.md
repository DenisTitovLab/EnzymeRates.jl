...
- Internal enumeration scratch (`_RawSpec`, `_RawStep`, `_RawAllostericSpec`, used by `_init_raw_specs` inside `src/mechanism_enumeration.jl`) is NOT part of the public API and is leading-underscore-private by convention.
...
- `EnzymeReaction` is a concrete (non-parametric) struct holding `reactants::Vector{ReactantAtoms}`, `regulators::Vector{RegulatorMults}`, and `allowed_catalytic_multiplicities::Vector{Int}`. Atom inventories live on `ReactantAtoms`, not on the reaction type parameter. (The earlier singleton-typed `EnzymeReactionLegacy{S,P,R,N}` was retired in Stage 7d.1.)
...
- `Step` is structurally-keyed: `==` and `hash` ignore `source_idx` (which is presentation metadata for parameter naming). RE binding steps canonicalize to put bound metabolite on the to-side; SS steps preserve user-source direction (CLAUDE.md "Canonical Step Form" invariant).
...
- `@generated` functions used for compile-time computation (`stoich_matrix`, `rate_equation`, `_kcat_forward`, `_dependent_param_exprs`, `parameters`, and the 5 zero-alloc accessors `substrates`/`products`/`regulators`/`reactions`/`enzyme_forms`).

### Parameter naming chokepoint

All `Parameter → Symbol` rendering in `src/` flows through one of two entry points sharing a private `_param_symbol` formatter family:
- **Value-context** `name(p::Parameter, m::Mechanism) → Symbol` — takes a constructed Parameter (carrying `step::Step` or `site::RegulatorySite + ligand::AllostericRegulator`) plus a Mechanism. Used everywhere a Step value is in scope.
- **Type/index-context** `name(::Type{P}, idx::Int [, state::Symbol]) → Symbol` for `P ∈ {Kd, Kiso, Kon, Kfor, Koff, Krev}` — takes a Parameter type and a kinetic-group rep-index. Used by `@generated` callers in `src/rate_eq_derivation.jl` that don't have a Step value in scope. (`Kreg`/`Keq`/`Etot`/`Lallo` are value-context only — they need ligand/site info or are stateless.)

`test/test_chokepoint.jl` enforces this — any direct `Symbol("K…")`/`Symbol("k…")`/`Symbol("V…")`/`Symbol("L…")` literal outside a `name(...)` or `_param_symbol(...)` body fails the build (AST walker, not regex). The future parameter-naming refactor (`:K1` → `:K_ATP`) changes a single function body.
...
- Both `AllostericMechanismSpec` (Dict storage) and `AllostericEnzymeMechanism` (Tuple type parameters) use **dense** storage — every kinetic group has an explicit `group_tags` entry; every ligand has an explicit `reg_ligand_tags` entry. The default tag is `:NonequalRT` but is stored explicitly. The `AllostericMechanismSpec` constructor validates density.
...
- **Internal scratch types**: `_RawSpec` / `_RawStep` / `_RawAllostericSpec` (leading-underscore-private) carry the same shape as the deleted spec types. They live inside `init_mechanisms(::EnzymeReaction)` so the heavy-pipeline algorithm doesn't change shape; the `_raw_from_mechanism` / `_mechanism_from_raw` converters bridge to `Mechanism` at the boundaries.
...
- `src/types.jl` — Concrete structs `EnzymeReaction`, `Mechanism`, `AllostericMechanism`, `Step`, `Species`, `Residual`, `ReactantAtoms`, `RegulatorMults`, `RegulatorySite`, `Metabolite` family (`Substrate`, `Product`, `CompetitiveInhibitor`, `AllostericRegulator`), `Parameter` family (`Kd`/`Kiso`/`Kon`/`Koff`/`Kfor`/`Krev`/`Kreg`/`Keq`/`Etot`/`Lallo`); singleton-type wrappers `EnzymeMechanism{Sig}` + `AllostericEnzymeMechanism{...}` with `_sig_of` / `_mechanism_from_sig` converters; 2-arg `EnzymeMechanism(metabolites, reactions)` legacy constructor (kept for the few opaque-form DSL fixtures that still need it); 3-arg `AllostericEnzymeMechanism(cm, cat_sites, reg_sites)` constructor with allosteric-state validation; accessors (`substrates`/`products`/`regulators`/`metabolites`/`reactions`/`equilibrium_steps`/`kinetic_group`/`kinetic_groups`/`steps_in_group`/`enzyme_forms`/`n_states`/`n_steps`/`stoich_matrix`/`enzyme_row_range`/`metabolite_row_range`/etc.) — most are `@generated` and walk `Sig` at compile time (0-alloc); the value-context Parameter-name chokepoint `name(p, m)` + type/index-context `name(::Type{P}, idx)` companion + private `_param_symbol` formatter; `RateEquationMode` hierarchy.
- `src/dsl.jl` — `@enzyme_reaction` (supports atom brackets); `@enzyme_mechanism` and `@allosteric_mechanism` accept the decomposed-Species call grammar `E(S)` / `E(A, B)` (parses to `Species([Substrate(:S)], :E)`) AND the legacy opaque-form bare-Symbol grammar (`:ES`) — dual-grammar transition. The parser distinguishes by AST: `Symbol` vs `Expr(:call, conformation, ligands...)`. New emission → `EnzymeMechanism(Mechanism(...))` when all step entries are decomposed-compatible; legacy emission → `EnzymeMechanism(metabolites, reactions)` when any opaque bound-form Symbol appears. `_is_conformation_shape` regex `^[A-Z][a-z0-9]*(_[a-z0-9]+)*$` distinguishes conformations from opaque bound-form names.
...
- `src/identify_rate_equation.jl` — `IdentifyRateEquationProblem{R<:EnzymeReaction, D<:NamedTuple}` (single constructor on `EnzymeReaction`); `identify_rate_equation(prob)` beam search + LOOCV; `_canonical_rate_eq_hash_data_impl_struct` (the struct-based canonical hash from Stage 6.1); `_project_cached_params` for fit reuse across hash-equivalent specs.
- `src/mechanism_enumeration.jl` — Building blocks: `init_mechanisms(reaction::EnzymeReaction)` → `Vector{Mechanism}`, `expand_mechanisms` → `Dict{Int, Vector{...}}`, `dedup!`; native unified expansion moves (`_expand_re_to_ss`, `_expand_split_kinetic_group`, `_expand_add_dead_end_regulator`, `_expand_to_allosteric`, `_expand_add_allosteric_regulator`, `_expand_change_allo_state`) — each dispatches on `Mechanism` / `AllostericMechanism`; mirror propagation implicit in kinetic-group atomicity. Internal scratch `_RawSpec` / `_RawStep` / `_RawAllostericSpec` (with `_init_raw_specs` / `_raw_from_mechanism` / `_mechanism_from_raw` converters) feed the heavy enumeration pipeline.
