...
### test_mechanism_enumeration.jl `@testset "Hash-equivalent mechanisms share fitted_params"` — assertion changed (not deleted)
- **Original**: `fitted_params(ldh_m_a) == fitted_params(ldh_m_b)` (raw symbol-tuple equality), which passed only because the deleted 2-arg `EnzymeMechanism(mets, rxns)` constructor stored steps in source order, giving both mechanisms rep-step names `(:K1,:K2,:K3,:K4,:k10f)`.
