...
- **OnlyA — live I-states re-derive natively; dead I-states use the reachability partition.** For a **live** I-cycle, the `:OnlyA` step is absent from the pruned I-graph and King–Altman re-derives the correct I-state law natively. For a **dead** I-cycle (an `:OnlyA` catalytic group breaks the cycle), naive pruning is *wrong*: it strands the now-unreachable downstream intermediate, whose steady-state weight pollutes `Q_I` with SS rate constants. The physically correct dead-state `Q_I` is the partition over forms **reachable from free E without the `:OnlyA` step** — exactly the reachability filter that monomial-zeroing (`_i_state_num_den_polys`) produces, and what the hand-derived `m_OnlyA_prod` analytical (`Q_cat_T = 1 + S/K1`) expects. **Implemented resolution:** gate on `_i_state_dead` — live → native `_state_rate_polys(am,:I)`; dead → reachability-filtered `_i_state_num_den_polys`, `N_I = 0`. Both are byte-identical to today and oracle-validated. *Follow-up (not D1):* a unified native path could prune `:OnlyA` steps **and** unreachable enzyme forms before King–Altman, making the dead-state partition native too and letting `_i_state_num_den_polys` be deleted; deferred to keep D1 on a verified state.
...
**Deletes** (live-path rename/reconstruction bookkeeping): `_A_rename_parameters`, `_a_to_i_rename`, `_I_rename_parameters`, `_raw_symbolic_rate_polys_allosteric`, `_dependent_param_exprs_allosteric`, `_synthesized_dep_i_names`, `_add_case_b_renames!`, `_i_state_referenced_syms`, most of the `_dependent_param_exprs(::AllostericEnzymeMechanism)` closure, and the re-inlined I-state polynomial construction inside `_kcat_forward` (which instead consumes the shared per-state derivation). Task 5 deletes each only after grep-confirming it is unreferenced, so the set is self-correcting.

**Stays** (irreducible MWC + dead-state reachability): `_reg_site_expr` and `Kreg` naming; `Lallo`; the one Case-B naming rule (§4); the `num_A + L·num_I` MWC combine; **and `_i_state_num_den_polys` + `_i_state_dead`** — retained as the dead-state reachability partition (§3), together with whatever A→I rename helper `_i_state_num_den_polys` still calls to name its surviving binding groups. Net deletion is therefore smaller than first projected; the live-state simplification and the elimination of the live-path desync-bug class still land, and the unified-native follow-up (§3) would delete the rest.
