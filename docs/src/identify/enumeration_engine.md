...
The candidate universe is a *product* of independent combinatorial choices, so it
grows explosively with the number of substrates, products, and regulators. Each
factor depends on the reaction's counts — `n_s` substrates, `n_p` products, `n_i`
competitive inhibitors, `n_a` allosteric regulators.

- **Catalytic binding topologies.** Ordered mechanisms fix a binding sequence; the
  random-order topology adds one more. The number of distinct sequences grows with
  `n_s!` and `n_p!`.
- **Substrate/product dead-end forms.** A dead-end form binds a mix of substrates
  and products at the catalytic site. Which reactants may co-occupy is a bipartite
  competition graph on `n_s × n_p` in which every substrate and every product
  touches at least one edge. The count is
  `N(n_s, n_p) = Σᵢⱼ (−1)^{i+j} C(n_s,i) C(n_p,j) 2^{(n_s−i)(n_p−j)}` — `N(1,1)=1`,
  `N(2,2)=7`, `N(3,3)=265`. `shared_catalytic_site` prunes exactly this choice.
- **The seven expansion moves**, each multiplying the count:
  rapid-equilibrium/steady-state flips (up to `2^{groups}`), kinetic-group splits,
  competitive-inhibitor sites (a second competition count over the `n_i` inhibitors
  and the enzyme forms they bind), promotion to allosteric, the `n_a` allosteric
  ligands and their `:OnlyA`/`:OnlyI` states, relaxations of a shared state to
...
competitive inhibitor, one allosteric regulator). The 55 seeds already bundle the
binding topologies and the `N(2,2)=7` dead-end forms. The seven moves then act on
each seed across a range of parameter counts — steady-state flips, splits, the
inhibitor site, promotion to allosteric, the regulator's `:OnlyA`/`:OnlyI` states,
a relaxation to independent A/I, and a site merge — multiplying the seeds
into the tens of thousands of candidate mechanisms. That figure is a calculated
estimate, not a measured count: enumerating the full set is exactly what the
package avoids. Structural deduplication and validity pruning trim the raw product,
but the growth *rate* is the point — one more reactant or regulator multiplies the
universe again.

Fitting even a fraction of that is infeasible, so the search is filtered, not
exhaustive. The beam keeps only the promising candidates at each parameter count;
`eq_complexity_filter` drops equations whose denominator is too dense to fit or
derive practically; required-regulator seeding skips the partially-regulated lower
shelf;
and `shared_catalytic_site` removes mechanisms a chemist already knows are wrong.
See [Best mechanism selection](@ref) for the filters that bound the search.
